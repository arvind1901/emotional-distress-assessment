"""
Emotional Distress Assessment — Flask Backend
Models loaded from models/ subfolder (trained by train_models.py)
"""
...
def calculate_caregiver_scores(answers, demographics):
    ...
    phys_fin_score     = (lost_income + med_expenses + perf + risk_job + missed_med + physically_exh + rest_sleep) / 7

    care_mgmt_score    = _get(answers, "difficult_to_manage")

    treatment_weights = {
        "Chemotherapy": 1.3, "Hormone Therapy": 0.7, "Surgery": 1.0,
        "Targeted Therapy": 1.2, "Radiation Therapy": 1.1, "Medication": 0.5
    }
    treatments       = demographics.get("treatments", []) or []
    treatment_score  = sum(w for t, w in treatment_weights.items() if t in treatments)

    gender = float(demographics.get("gender", 0))

    # Feature vector must match training: [sleep_score, Financial_Score, Social_Score, Anxiety_Score, Care_Management_Score, Gender]
...
def calculate_patient_scores(answers, demographics):
    ...
    # Feature vector must match training: [Patient_Sleep_Score, Patient_Physical_Symptom_Score_Sub, Patient_Emotional_Score_Sub, Age, Gender]
...
class ModelManager:
    def __init__(self):
        ...
        self.care_pipe  = pickle.load(open(CARE_MODEL,  "rb"))
        self.care_scaler= pickle.load(open(CARE_SCALER, "rb"))
        self.pat_pipe   = pickle.load(open(PAT_MODEL,   "rb"))
        self.pat_scaler = pickle.load(open(PAT_SCALER,  "rb"))
        print("✅ Models loaded successfully.")

    def _predict(self, pipe, features):
    ...
    def predict_caregiver(self, features):
        return self._predict(self.care_pipe, features)

    def predict_patient(self, features):
        return self._predict(self.pat_pipe, features)
...
def predict():
    try:
        ...
        if role == "caregiver":
            ...
            pred, conf       = model_manager.predict_caregiver(features)
        else:
            features, scores = calculate_patient_scores(answers, demographics)
            pred, conf       = model_manager.predict_patient(features)
...
def health():
    ...
    return jsonify({
        ...
        "caregiver_model":     model_manager.care_pipe  is not None,
        "patient_model":       model_manager.pat_pipe   is not None,
